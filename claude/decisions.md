...
- **Flush confirm gains "Discard & load".** A failed mid-transform leaves rows in staging (the transaction rolls back the truncate). Loading a new file shows the confirm with three choices now: **Flush & load** (retry the transform on the old rows, then load), **Discard & load** (`TRUNCATE staging` then load on a clean table — for a known-bad batch), **Cancel**. `table_load` handles `discard=1`; `_load_confirm.html` third button.
- **Quarantine / Waiting lineage boxes light up only once a row lands there** — not while any transform runs. `_pipeline_stage_state` returns `waiting` / `quarantine` = `done` iff the diversion table holds rows (or the last run routed some), else None; no `running` pulse. The `.div.q` / `.div.w` connectors get a static (non-animated) highlight when their box is lit.
- **Waiting / Quarantine surfaces show row counts, not batch counts.** KPI cards relabelled **"waiting rows" / "quarantine rows"** (`_count_safe` of the physical `waiting.<t>` / `quarantine.<t>` table); lineage sub-labels `N held` / `N bad`; the compare-grid `rows` metric and "Rows in batch" render compact (`sum`/`min`/`max` stay literal for precision). The per-table "Waiting batches" / "Quarantine batches" cards keep their **batch** count (they list batches).
...
- **Waiting batches gain a `merge` outcome** — "keep both": insert the held rows into production alongside the existing ones (no delete, `restated = false`, `status = 'merged'`), for a key collision that is a genuine additional record. Third button next to Approve/Reject on the list + detail screens; `resolve.merge_waiting` (shares `_admit_waiting` with `approve_waiting`); `POST /waiting/{w}/merge`; `odin waiting merge`; `sql/005_waiting_merge.sql` widens the status CHECK.
- **Self-refreshing panels morph instead of replace.** The runs panel / lineage / KPI cards polled with `hx-swap="outerHTML"` every 3s, tearing down the subtree each time — the running-row sweep animation restarted and the table flickered. Vendored `idiomorph-ext.min.js` (0.7.3, ~10 KB) after htmx, `<body hx-ext="morph">`, and the three partials now use `hx-swap="morph:outerHTML"` — DOM is diff-patched, so animations and scroll position survive. Poll interval 3s → 4s.
- **Row counts render compact** (`app._compact` Jinja `compact` filter): `1000` → `1k`, `1234` → `1.23k`, `2_500_000` → `2.5M` (≤2 decimals, trailing zeros trimmed); exact value on `title=` hover. Applied to the KPI stat cards (`_data_stats.html`), the deck's `Rows → production` KPI, lineage node sub-labels, `numcell` (runs table + waiting/quarantine panels), and the per-batch `Rows` displays — so growing data doesn't blow out card / cell width.
