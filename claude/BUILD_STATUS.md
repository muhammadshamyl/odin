...
| 1 · Extraction | 🟡 | file + **RDBMS (Postgres)** paths | NoSQL, scheduler, `extraction_control` | file / batched DB query → CSV → same tail |
...
| 1.5 | RDBMS Connector | 🟡 | Slice 2, **all 4 phases built** (PostgreSQL) (`connectors/rdbms.py`, PostgreSQL only): `probe` (connect + `version()` + schema list, cleaned driver errors), `save_connection` / `get_connection` / `connection_meta` — credentials in a `secret` schema, password `pgcrypto`-encrypted (`sql/006_rdbms.sql`, key = `settings.rdbms_secret_key`). Web: `/onboard` reflowed to a 2-column source picker (File | Relational database), `POST /onboard/rdbms/test`, `/onboard/rdbms/{cid}/schemas` (visual schema-tile picker) → `/{schema}/tables` (radial FK **cobweb** — `list_tables` + `fk_edges` from `pg_class`/`pg_constraint`, `_cobweb_layout` places nodes on a ring with FK arcs bowed to centre; click a node → JS lights incident threads + htmx-loads a `peek` panel, `SELECT * … LIMIT 20` + column types; a live substring filter re-lays-out). `/{schema}/{table}/configure` (pick a tenure/date column + optional static range filter — calendar for dates) → `POST …/preview` runs `SELECT … LIMIT 200` (filter applied) and hands into the **existing `preview.html`** with production types **pre-filled** from the source schema (`rdbms.column_tokens`) + RDBMS hidden fields; `onboard_create` branches to `registry.onboard_rdbms_source` (refactored shared `_onboard`, `src_type='RDBMS'`, `registry_rdbms_source` sidecar via `sql/007`). Reserved-name guard (`batch_id`, `load_date`, … `nk`) on both paths. `connectors/rdbms.extract_to_csv` streams the source via a server-side cursor (`WHERE` = static filter [+ tenure window]) → CSV → `extract.run_extract_rdbms` runs the exact file tail (land → Check 1 → COPY → transform) under one `run_id` (`dag_id='extract_rdbms'`); `jobs` kind `ingest_rdbms`; the table page shows a **Full / Tenure** run form instead of the dropzone (`POST /t/{s}/{t}/run-rdbms`). `casts.cast_select` now `NULLIF(btrim(col),'')::type` so an empty nullable typed cell becomes NULL (also a latent CSV fix). Standalone CSV/TXT path ✅ via `connectors/file.py`. |
